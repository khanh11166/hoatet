...
function pt_like_scripts() {
	wp_enqueue_script( 'pt_like_post', get_template_directory_uri().''.str_replace(str_replace('\\', '/', get_template_directory()), '',str_replace('\\', '/', __DIR__)).'/js/post-like.js', array('jquery'), '1.0', true );
 ...
 */
function pt_like_shortcode() {
	return pt_get_like_button( get_the_ID() );
}
add_shortcode('pt-like-button', 'pt_like_shortcode');
	...
	if ( is_user_logged_in() ) { // user is logged in
		...
		if ( $count > 0 ) {
			...
			<div class="favourite-posts">
				<h3><?php _e( 'You Like:', 'plumtree' ); ?></h3>
				<ul>
		...
		<div class="favourite-posts">
			<h3><?php _e( 'Nothing yet', 'plumtree' ); ?></h3>
		</div>
	<?php }
}

/* User Likes Widget */
add_action( 'widgets_init', create_function( '', 'register_widget( "pt_user_likes_widget" );' ) );

class pt_user_likes_widget extends WP_Widget {
	
	public function __construct() {
		parent::__construct(
	 		'pt_user_likes_widget', // Base ID
			__('PT User Likes Widget', 'plumtree'), // Name
 ...
 * Markup assumes sidebar/widget usage
 */
add_action( 'widgets_init', create_function( '', 'register_widget( "pt_popular_posts_widget" );' ) );

class pt_popular_posts_widget extends WP_Widget {
	
	public function __construct() {
		parent::__construct(
	 		'pt_popular_posts_widget', // Base ID
			__('PT Popular Posts Widget', 'plumtree'), // Name
