		...
		if ($content) { $html .= '<h3 class="socials-title pt-content-title">'.esc_attr($content).'</h3>'; }
			...
			foreach ( $services as $service_id => $service ) {
				if ( isset($service['url']) && $service['url'] != '' ) {
					...
					$html .= '<li class="listing listing-' . esc_attr($service_id) . '">';
					$html .= '<a href="' . esc_url($service['url']) . '" target="_blank" title="' . esc_attr($text) . '">';
					$html .= '<i class="fa fa-' . esc_attr($service['icon']) . ' fa-2x"></i>';
					$html .= '</a></li>';
				}
			}
			$html .= '</ul>';
