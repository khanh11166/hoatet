...
class MediauploadField extends FormField{
	...
	public function setView() {
		...
		if (!isset($button_label)) $button_label = __('Upload', 'ptpanel');
...
class OptionFactory{
	...
	public static function create($option_id, $option_name, FieldType $type, $section_id, $option_label = '', $additional_params = array(), $in_customizer = false){
		...
		foreach(self::$optionCollection as $field) {
			if ($option_name == $field->getName()) throw new Exception(sprintf(__('Option with name: %s already exists', 'ptpanel'), $option_name));
		}
...
class PanelSectionFactory{
	...
	public static function registerSection($sectionId){
		...
		if (in_array($sectionId, self::$availableSections)) throw new Exception(sprintf(__('Sections with ID: %s already exists', 'ptpanel'), $sectionId));
...
class PTPanel{
	...
	function loadPanel(){
		if (!current_user_can('manage_options'))  {
			wp_die( __('You do not have sufficient permissions to access this page.', 'ptpanel') );
		}
		$this->displayPanel();
	}
	...
	function displayPanel(){
		...
		echo '<p class="submit"><button type="submit" class="btn btn-primary" id="submit" name="submit">'.__('Save Changes', 'ptpanel').'</button></p>';
		echo '</div></form></div>';
